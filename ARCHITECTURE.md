...
- The external automation API is currently a design package under `docs/api/`, not a deployed runtime. Its generated OpenAPI contract and schema catalog define a future gateway in front of Postgres and the existing Edge Functions. The gateway must issue scoped credentials, resolve the application role and tenant, preserve RLS, validate fields, require idempotency/optimistic concurrency/confirmation where specified and append audit events. Third-party agents never receive the Supabase `service_role` key or unrestricted raw database access.
- New-client AI onboarding is initially bound to the authenticated profile. The onboarding Edge Function resolves the linked client record server-side and gives the AI the authoritative client and business names, while the activity, need and project scope still come only from what the client shares. Submission creates or updates a pre-project lead conversation and does not create a project. Only an agency-admin promotion creates the exact project, retry-safely transfers the complete transcript, internal notes, structured brief, flow and specification drafts, and binds subsequent project/MVP work to that project. MVP generation remains agency-only.
...
- An authenticated client uses only the simple home workspace. It shows the current project stage, project chat, client-visible files and shared client MVP versions directly; it does not expose the flow-canvas controls or a link into the Advanced project workspace. Realtime updates on `prototype_versions` refresh a newly shared MVP without creating a parallel client-side record.
