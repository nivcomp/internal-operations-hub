...
- Client process explanations are a presentation over those existing LiveFlow/artifact nodes, not another persisted workflow. `ProjectFlowCanvas` and chat flow artifacts share `ProcessNode`, `ProcessNodeDetails`, `clientProcessExplanation` and the client-safe glossary; they never invent project status or implementation dependencies.
- Client project chats opt into a final `clientSafe` presentation boundary that suppresses internal drafts, rejected-action reasons, internal cost and margin effects even if an upstream payload accidentally contains those fields. Server role filtering and RLS remain the primary authorization boundary.
