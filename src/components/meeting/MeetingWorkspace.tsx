...
export function MeetingWorkspace({ projectId, projectName, clientName, companyName, onSaveExit, onFinished }: Props) {
  ...
  return <div className="meeting-workspace" dir="rtl">
    <header className="card meeting-head"><div><p className="eyebrow">{companyName || "פגישת לקוח"}</p><h2>חדר אפיון — {projectName}</h2><p>{clientName ? `איש קשר: ${clientName} · ` : ""}מצב: {meeting.status === "active" ? "פגישה פעילה" : "הפגישה הסתיימה"}</p><div className="meeting-time-strip"><span><small>התחלה</small><strong>{new Date(meeting.started_at).toLocaleTimeString("he-IL", { hour: "2-digit", minute: "2-digit" })}</strong></span><span><small>{meeting.ended_at ? "סיום" : "זמן חי"}</small><strong>{meeting.ended_at ? new Date(meeting.ended_at).toLocaleTimeString("he-IL", { hour: "2-digit", minute: "2-digit" }) : "● פעיל"}</strong></span><span><small>משך הפגישה</small><strong>{formatDuration(elapsed)}</strong></span>{meetingCharge ? <span><small>שעות אפיון</small><strong>{meetingCharge.billable_hours}</strong></span> : null}</div><small>{savedAt ? `נשמר לאחרונה ${savedAt.toLocaleTimeString("he-IL")}` : "המידע נשמר ב-Supabase"}</small></div><div className="action-row"><button onClick={() => void refresh().then(() => setSavedAt(new Date()))}>שמור</button><button onClick={() => setDocumentsOpen((value) => !value)}>מסמכי הפרויקט</button><button onClick={() => void shareClientLink()} disabled={linkState === "loading"}>{linkState === "loading" ? "מכין לינק…" : linkState === "copied" ? "הלינק הועתק" : "לינק להמשך הפרויקט"}</button>{onSaveExit ? <button onClick={onSaveExit}>שמור וצא</button> : null}{meeting.status === "active" ? <button className="primary-button" onClick={openFinish}>סיים פגישה</button> : null}</div>{linkState === "error" ? <p className="form-error">לא הצלחנו להפיק לינק להמשך האפיון. ודא שלפרויקט משויך לקוח.</p> : null}{needsEmail ? <div className="meeting-client-link"><small>אין מייל שמור ללקוח. הזינו מייל כדי להפיק את הלינק:</small><input dir="ltr" type="email" value={linkEmail} onChange={(event) => setLinkEmail(event.target.value)} placeholder="client@example.com" /><div className="action-row"><button onClick={() => void shareClientLink(linkEmail)} disabled={linkState === "loading"}>הפק לינק</button></div></div> : null}{clientLink ? <div className="meeting-client-link"><small>לינק חד־פעמי להמשך הפרויקט הזה — הלקוח רק בוחר סיסמה ונכנס:</small><input readOnly dir="ltr" value={clientLink} onFocus={(event) => event.currentTarget.select()} /><div className="action-row"><button onClick={() => void copyToClipboard(clientLink).then(() => setLinkState("copied"))}>העתק לינק</button><a className="button-link" href={clientLink} target="_blank" rel="noreferrer">פתח</a><a className="button-link" href={`https://wa.me/?text=${encodeURIComponent(`היי, זה הלינק להמשך האפיון של הפרויקט ${projectName}: ${clientLink}`)}`} target="_blank" rel="noreferrer">שלח ב‑WhatsApp</a></div></div> : null}</header>
    ...
    <div className="guided-meeting-layout">
      ...
      <aside className="guided-summary card"><h3>מצב הפרויקט בלייב</h3><dl><div><dt>מאושר</dt><dd>{approved}</dd></div><div><dt>עוד לבדיקה</dt><dd>{incomplete}</dd></div><div><dt>חומרים</dt><dd>{sources.length}</dd></div></dl><section className="client-safe-estimate"><h4>אומדן משותף</h4>{estimate?.client_visible ? <><p><strong>{estimate.estimated_hours_min}–{estimate.estimated_hours_max}</strong> שעות</p>{estimate.final_fixed_price && estimate.approved_by_yaniv ? <p>מחיר קבוע מאושר: <strong>{money(estimate.final_fixed_price, estimate.currency)}</strong></p> : <small>בפגישה מוצגות שעות בלבד. מחיר יוצג לאחר אישור מחיר קבוע.</small>}</> : <p className="form-note">האומדן יוצג כאן רק לאחר אישור לשיתוף עם הלקוח.</p>}</section><button onClick={() => setManualOpen((value) => !value)}>{manualOpen ? "הסתר חומרים" : "קבצים וחומרי הפגישה"}</button></aside>
    </div>
