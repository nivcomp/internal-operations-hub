...
export function DashboardPage({
  ...
  const projectsNeedingAttention = projects.filter(
    (project) =>
      !canWorkStart(project, scopes) ||
      ["waiting_for_agency_pricing", "waiting_for_client_approval"].includes(project.status),
  );

  const marginPercent = metrics.revenue ? Math.round((metrics.margin / metrics.revenue) * 100) : 0;
  ...
  return (
    <>
      ...
      <section className="stats-grid stats-grid-primary">
        <StatCard label="Waiting for pricing" value={metrics.waitingPricing} tone={metrics.waitingPricing > 0 ? "warning" : "default"} />
        <StatCard label="Waiting for client approval" value={metrics.waitingApproval} tone={metrics.waitingApproval > 0 ? "warning" : "default"} />
        <StatCard label="Waiting for payment" value={metrics.waitingPayment} tone={metrics.waitingPayment > 0 ? "warning" : "default"} />
            ...
            <tbody>
              {projectsNeedingAttention.map((project) => (
                <tr key={project.id} className="clickable-row" onClick={() => onProjectSelect(project.id)}>
                  ...
                  <td>{canWorkStart(project, scopes) ? "Ready" : "Blocked until payment or hours"}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      <section className="financial-overview">
        ...
        <div className="financial-grid">
          <div className="financial-item"><span>Total client revenue</span><strong>{currency.format(metrics.revenue)}</strong></div>
          <div className="financial-item"><span>Total supplier cost</span><strong>{currency.format(metrics.cost)}</strong></div>
          <div className="financial-item"><span>Estimated margin</span><strong>{currency.format(metrics.margin)}</strong></div>
          <div className="financial-item"><span>Owed to suppliers</span><strong>{currency.format(metrics.supplierOwed)}</strong></div>
        </div>
      </section>
