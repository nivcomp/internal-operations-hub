...
const openapi = {
  ...
  info: {
    ...
    description: "Design contract for a gateway that is not implemented yet. Do not point third-party agents at the Supabase service_role. The gateway must enforce scopes, application roles, tenant/row boundaries, RLS-equivalent checks, confirmations, idempotency, and audit logs.",
  },
  servers: [
  ...
  security: [{ oauth2: ["data.read"] }, { bearerAuth: [] }],
  paths: {
    "/v1/me": {
    ...
    "/v1/schema/tables": {
      get: { tags: ["Schema"], operationId: "listTables", summary: "List accessible tables and capabilities", security: [{ oauth2: ["schema.read"] }], responses: { "200": { description: "Tables filtered to the caller's effective permissions", content: { "application/json": { schema: { type: "object", properties: { data: { type: "array", items: { $ref: "#/components/schemas/TableSummary" } } }, required: ["data"] } } } }, ...standardErrors } },
    },
    "/v1/schema/tables/{table}": {
      get: { tags: ["Schema"], operationId: "getTableSchema", summary: "Get fields, relations, and permitted operations for one table", security: [{ oauth2: ["schema.read"] }], parameters: [tableParameter], responses: { "200": { description: "Table schema", content: { "application/json": { schema: { $ref: "#/components/schemas/TableSchema" } } } }, "404": { $ref: "#/components/responses/NotFound" }, ...standardErrors } },
    },
    "/v1/data/{table}": {
      get: {
        tags: ["Data"], operationId: "listRecords", summary: "List row-scoped records", security: [{ oauth2: ["data.read"] }],
      ...
      post: {
        tags: ["Data"], operationId: "createRecord", summary: "Create a record when direct creation is permitted", security: [{ oauth2: ["data.write"] }],
    ...
    "/v1/data/{table}/{recordId}": {
      get: { tags: ["Data"], operationId: "getRecord", summary: "Read one row-scoped record", security: [{ oauth2: ["data.read"] }], parameters: [tableParameter, recordIdParameter], responses: { "200": { description: "Record", headers: { ETag: { schema: { type: "string" } } }, content: { "application/json": { schema: { $ref: "#/components/schemas/RecordResult" } } } }, "404": { $ref: "#/components/responses/NotFound" }, ...standardErrors } },
      patch: { tags: ["Data"], operationId: "updateRecord", summary: "Patch a permitted record", security: [{ oauth2: ["data.write"] }], parameters: [tableParameter, recordIdParameter, { $ref: "#/components/parameters/IfMatch" }, { $ref: "#/components/parameters/IdempotencyKey" }], requestBody: { required: true, content: { "application/merge-patch+json": { schema: { $ref: "#/components/schemas/RecordWrite" } } } }, responses: { "200": { description: "Updated", content: { "application/json": { schema: { $ref: "#/components/schemas/RecordResult" } } } }, "404": { $ref: "#/components/responses/NotFound" }, "412": { $ref: "#/components/responses/PreconditionFailed" }, ...standardErrors } },
      delete: { tags: ["Data"], operationId: "deleteRecord", summary: "Guarded delete; unavailable for append-only or protected business records", security: [{ oauth2: ["data.delete"] }], parameters: [tableParameter, recordIdParameter, { $ref: "#/components/parameters/IfMatch" }, { $ref: "#/components/parameters/IdempotencyKey" }, { name: "X-Confirm-Delete", in: "header", required: true, schema: { type: "string", const: "DELETE" } }], requestBody: { required: true, content: { "application/json": { schema: { type: "object", properties: { reason: { type: "string", minLength: 3 }, mode: { type: "string", enum: ["soft", "hard"], default: "soft" } }, required: ["reason"], additionalProperties: false } } } }, responses: { "204": { description: "Deleted or archived" }, "404": { $ref: "#/components/responses/NotFound" }, "412": { $ref: "#/components/responses/PreconditionFailed" }, ...standardErrors } },
    },
    "/v1/actions/{service}/{action}": {
      post: {
        tags: ["Actions"], operationId: "executeBusinessAction", summary: "Execute a guarded business operation", security: [{ oauth2: ["actions.execute"] }],
        parameters: [
    ...
    "/v1/audit-events": {
      get: { tags: ["Audit"], operationId: "listAuditEvents", summary: "List immutable integration audit events", security: [{ oauth2: ["audit.read"] }], parameters: [{ name: "cursor", in: "query", schema: { type: "string" } }, { name: "limit", in: "query", schema: { type: "integer", minimum: 1, maximum: 200, default: 50 } }], responses: { "200": { description: "Audit page", content: { "application/json": { schema: { $ref: "#/components/schemas/RecordPage" } } } }, ...standardErrors } },
    },
  },
  components: {
    securitySchemes: {
      oauth2: { type: "oauth2", description: "Preferred for server-to-server Skills. Issue short-lived, scoped tokens.", flows: { clientCredentials: { tokenUrl: "https://api.YOUR_DOMAIN.example/oauth/token", scopes: { "schema.read": "Read schema metadata", "data.read": "Read permitted rows", "data.write": "Create/update permitted rows", "data.delete": "Request guarded deletes", "actions.execute": "Execute permitted business actions", "audit.read": "Read integration audit events" } } } },
...
writeFileSync(resolve(apiDir, "ai-skill-input.json"), `${JSON.stringify({
  packageFormat: "client-to-scope-ai-skill-input/v1",
  purpose: "Single-file source package for generating an API connector or AI Skill after the gateway described by the OpenAPI contract is implemented.",
  status: "design_contract_not_live_api",
  generationPrompt: [
    ...
    "Use OAuth 2.0 client credentials with least-privilege scopes. Keep API_BASE_URL, CLIENT_ID, and CLIENT_SECRET in environment variables or a secret manager. Never request, store, print, or expose a Supabase service_role key.",
    ...
    "Clearly state that this package describes a gateway that must be deployed before the generated Skill can connect.",
  ].join("\n"),
  openapi,
